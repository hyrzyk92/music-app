<script setup lang="ts">
defineProps<{
  categoryDatas: any[];
}>();
</script>

<template>
  <div class="category-wrap">
    <div class="category">
      <div class="sub-category">
        <a href="/" class="entry">
          <span class="category-icon channel"></span>
          <span class="category-title">频道</span>
        </a>
        <a href="/" class="entry">
          <span class="category-icon all"></span>
          <span class="category-title">分类</span>
        </a>
        <a href="/" class="entry">
          <span class="category-icon rank"></span>
          <span class="category-title">排行榜</span>
        </a>
      </div>
      <div class="recommend">
        <div
          class="recommend-item"
          v-for="group in categoryDatas"
          :key="group.id"
        >
          <div class="recommend-item-title">
            <a :href="group.groupLink">{{ group.groupName }}</a>
          </div>
          <div class="recommend-item-content">
            <span
              class="pill"
              v-for="channel in group.channels"
              :key="channel.id"
            >
              <a :href="channel.channelLink">{{ channel.channelName }}</a>
            </span>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<style scoped>
.category-wrap {
  width: 1080px;
  margin: 20px auto 0;
}
.category {
  height: 120px;
  padding: 20px 0 20px 0;
}
.sub-category {
  display: inline-block;
}
.entry {
  display: inline-block;
  margin: 10px 24px auto;
  line-height: 1.4;
  font-size: 14px;
  color: #40404c;
}
.category-icon {
  display: block;
  width: 18px;
  height: 18px;
  margin: auto;
  background-repeat: no-repeat;
  background-size: 100%;
}
.channel {
  background-image: url("data:image/png;base64,");
}
.all {
  background-image: url("data:image/png;base64,");
}
.rank {
  background-image: url("data:image/png;base64,");
}
.recommend {
  display: inline-block;
  width: 838px;
}
.recommend-item{
    display: inline-block;
    width: 206px;
}
.recommend-item-title {
    display: inline-block;
    width: 56px;
    text-align: right;
    color: #40404c;
    font-size: 14px;
    margin-right: 12px;
    position: relative;
}
.recommend-item-title a {
    color: #40404c;
}
.recommend-item-title::before {
    content: '';
    display: block;
    width: 2px;
    height: 10px;
    background-color: #fc5832;
    position: absolute;
    left: 100%;
    top: 0;
    bottom: 0;
    margin: auto 8px;
}
.recommend-item-content {
    display: inline-block;
}
.pill {
    display: inline-block;
    padding: 0 10px 5px;
}
.pill a {
    color: #72727b;
}
.recommend-item-title a:hover, .pill a:hover {
    color: #f86442;
}
</style>
